...
let int_of_imm = function
  | Op.Reg _ | Op.Fmm _ -> sparc_fail "imm operand expected"
  | Op.Imm x -> match Imm.to_int x with
    | Some x -> x
    | None -> sparc_fail "failed to convert imm operand to int"
...
let reg find signed op = match op with
  | Op.Imm _ | Op.Fmm _ -> sparc_fail "reg operand expected"
