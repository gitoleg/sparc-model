...
let var_bitwidth v =
  ...
  | Type.Imm w -> w
  | _ ->
    sparc_fail "variable %s doesn't has notion of bitwidth" (Var.name v)
...
let var_of_exp e = match e.body with
  | Vars (v,_) -> v
  | _ -> sparc_fail "variable expected"
...
module Exp = struct
  ...
  let of_vars vars = match vars with
    | [] -> sparc_fail "can't constuct an expression from empty var list"
    | v :: vars ->
...
module Translate = struct
  ...
  let rec move lhs rhs =
    ...
    | _ -> sparc_fail "unexpected left side of :="
